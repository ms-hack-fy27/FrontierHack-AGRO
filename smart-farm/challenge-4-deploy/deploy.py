...
MONITOR_AGENT_NAME = "crop-health-monitor-agent"
ADVISOR_AGENT_NAME = "agricultural-advisor-agent"
...
def ensure_agents_deployed() -> tuple:
    ...
    tool = FunctionTool(name="check_crop_health", description="Compare all four crop metrics with thresholds.", parameters={"type": "object", "properties": {"zone_id": {"type": "string"}}, "required": ["zone_id"]}, strict=False)
    ...
    if MONITOR_AGENT_NAME not in existing:
        client.agents.create_version(agent_name=MONITOR_AGENT_NAME, definition=PromptAgentDefinition(model=MODEL_DEPLOYMENT_NAME, instructions="You are the GreenRise crop health monitor. Use check_crop_health for every requested zone and return structured status, thresholds, anomalies, and next steps.", tools=[tool]))
    ...
    try:
        ...
        while True:
            ...
            for call in calls:
                ...
                outputs.append(FunctionCallOutput(type="function_call_output", call_id=call.call_id, output=check_crop_health(args["zone_id"])))
...
def run_portal_workflow(workflow_name: str) -> str:
    ...
    conversation = openai_client.conversations.create()
    try:
        response = openai_client.responses.create(input="Analyze this embedded smart farm data directly; do not call check_crop_health. Detect anomalies, then recommend agronomic actions.\n" + embedded, conversation=conversation.id, extra_body={"agent_reference": {"name": workflow_name, "type": "agent_reference"}}, background=True)
        for _ in range(12):
            time.sleep(8)
